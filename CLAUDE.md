...
`max_turns` bounds **one question**, not the conversation. `BaseAgent._chat` counts AIMessages back to the last `HumanMessage` (`_current_turn`), because `state.messages` is the whole checkpointed thread: counting all of it turned the limit into a per-conversation budget, so a `/match` chat that spent 12 assistant turns across five questions then killed every further question in ~40ms — `agent_max_turns_reached` before any LLM call, empty answer, `status='completed'`, no error anywhere. Regression-tested in `tests/test_agent_turns.py`. The crawler is unaffected either way: one crawl is one question per thread. A turn that still ends with nothing now writes an explanatory answer rather than a blank bubble.
