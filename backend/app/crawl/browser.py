...
class BrowserManager:
    ...
    def search_traffic(self, keyword: str) -> list[dict]:
        """Search all captured traffic for a keyword in URL, request body, or response body."""
        keyword_lower = keyword.lower()
        results = []
        for i, r in enumerate(self.captured):
            matches = []
            # Search URL
            if keyword_lower in r["url"].lower():
                matches.append("url")
            # Search request body
            if r.get("request_body"):
                req_str = json.dumps(r["request_body"], ensure_ascii=False).lower()
                if keyword_lower in req_str:
                    matches.append("request_body")
            # Search response body (full, not just preview)
            resp_str = json.dumps(r["response_body_preview"], ensure_ascii=False).lower()
            if keyword_lower in resp_str:
