...
Ported verbatim from the fastapi-langgraph-agent template.
"""
...
class LLMService:
    ...
    def set_model(self, model_name: str) -> "LLMService":
        """Point this service at a specific registry model.

        Added on top of the template so each agent can pick its own model while
        still sharing the retry and fallback machinery.

        Args:
            model_name: Name of a model in the registry.
        ...
        if model_name not in all_names:
            raise ValueError(f"model '{model_name}' not found in registry. available models: {', '.join(all_names)}")
