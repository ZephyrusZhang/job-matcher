...
from app.crawl.pipeline import run_crawler, store_jobs
...
class CrawlService:
    def __init__(self, company_service: CompanyService, config: AppConfig):
        ...
        self.config = config
        # task_id → threading.Event, set() means "please cancel"
    ...
    async def cancel(self, db: aiosqlite.Connection, task_id: str) -> CrawlTaskOut:
        ...
        # Signal the crawl thread to stop
    ...
    def _run_crawl_sync(
        ...
        """Synchronous wrapper that runs in a thread."""
        try:
            asyncio.run(
                self._run_crawl(task_id, company_id, career_url, db_path, cancel_event)
            )
        finally:
    ...
    async def _run_crawl(
        ...
        try:
            ...
            logger.info(f"Crawl started: company={company_id}, task={task_id}")

            # Run the crawler (blocking call via AgentRunner)
            ...
            if cancel_event.is_set():
                logger.info(f"Crawl cancelled after agent phase: task={task_id}")
                return

            # Store jobs into DB (with cancellation support)
            jobs_found, jobs_new, jobs_updated = await store_jobs(
                db, raw_jobs, company_id, cancel_event
            )

            # If cancelled during store, still record partial progress
